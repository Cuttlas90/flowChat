  ...
  async ({ uuid, userAddress, contactAddress, message, timestamp }) => {
    const transactionId = await fcl.mutate({
      ...
      `, args: (arg, t) => [
        ...
        arg(Math.ceil(timestamp / 1000) + '.0', t.UFix64),
  ...
  async ({ uuid, userAddress, channelTyple, subscribeFee }) => {
    const transactionId = await fcl.mutate({
      ...
      `, args: (arg, t) => [
        ...
        arg(Math.ceil(subscribeFee / 1) + '.0', t.UFix64),
      ],
      payer: fcl.authz,
      proposer: fcl.authz,
      authorizations: [fcl.authz],
    ...
    const transactionId = await fcl.mutate({
      ...
      transaction(Amount: UFix64, ChannelId: String) {
        ...
        prepare(signer: AuthAccount) {
            ...
            MessangerTest4.joinToPrivateChannel(sender: signer.address,channelId: channelId, amount: Amount, payment: <- self.sentVault)
    }
    }
      `, args: (arg, t) => [
        arg(t.UFix64(amount), t.UFix64),
        arg(channelId, t.String)
      ],
      payer: fcl.authz,
      proposer: fcl.authz,
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(createChannel.fulfilled, (state, action) => {
        ...
        state.user.createChannel.txId = action.payload;
      })
    ...
    builder
      .addCase(joinToPublicChannel.pending, (state) => {
        state.user.joinToPublicChannel.status = 'loading';
        state.user.joinToPublicChannel.error = '';
      })
      .addCase(joinToPublicChannel.fulfilled, (state, action) => {
        state.user.joinToPublicChannel.status = 'idle';
        state.user.joinToPublicChannel.txId = action.payload;
      })
      .addCase(joinToPublicChannel.rejected, (state, action) => {
        state.user.joinToPublicChannel.status = 'rejected';
        state.user.joinToPublicChannel.error = action?.error;
      });
    builder
      .addCase(joinToPrivateChannel.pending, (state) => {
        state.user.joinToPrivateChannel.status = 'loading';
        state.user.joinToPrivateChannel.error = '';
      })
      .addCase(joinToPrivateChannel.fulfilled, (state, action) => {
        state.user.joinToPrivateChannel.status = 'idle';
        state.user.joinToPrivateChannel.txId = action.payload;
      })
      .addCase(joinToPrivateChannel.rejected, (state, action) => {
        state.user.joinToPrivateChannel.status = 'rejected';
        state.user.joinToPrivateChannel.error = action?.error;
      });
    builder
        ...
        if (action.payload?.message) {
          ...
          if (sendMessageList) {
            ...
            for (let uuid of sendmessagUuIds) {
              ...
              let finded = messges.find(item => Number(item.timestamp) === Math.ceil(timestamp / 1000));
              if (finded) {
    ...
    builder
      ...
      .addCase(getChannelChat.fulfilled, (state, action) => {
        state.user.getChannelChat.messageList[action.meta.arg.channelId] = { ...{}, "messages": action.payload, status: "idle", error: "" };
        if (action.payload?.message) {
          ...
          if (sendMessageList) {
            ...
            for (let timsMessage of sendmessagTimes) {
              let timestamp = sendMessageList[timsMessage].timestamp;
              let finded = messges.find(item => Number(item.timestamp) === Math.ceil(timestamp / 1000));
              if (finded) {
    ...
    builder
      .addCase(sendChannelMessage.pending, (state, action) => {
        state.user.sendChannelMessage.sendList[action.meta.arg.channel] = { ...state.user.sendChannelMessage.sendList[action.meta.arg.channel], [action.meta.arg.timestamp]: { ...action.meta.arg, "txId": action.payload, "status": "loading" } };
      })
      .addCase(sendChannelMessage.fulfilled, (state, action) => {
        state.user.sendChannelMessage.sendList[action.meta.arg.channel] = { ...state.user.sendChannelMessage.sendList[action.meta.arg.channel], [action.meta.arg.timestamp]: { ...action.meta.arg, "txId": action.payload, "status": "idle" } };
      })
      .addCase(sendChannelMessage.rejected, (state, action) => {
        state.user.sendChannelMessage.sendList[action.meta.arg.channel] = { ...state.user.sendChannelMessage.sendList[action.meta.arg.channel], [action.meta.arg.timestamp]: { ...action.meta.arg, "txId": action.payload, "status": "rejected" } };
      });
  },
});
